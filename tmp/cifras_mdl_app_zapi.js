...
async function notifyContactsZAPI(songId) {
  ...
  for (const contact of contacts) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`Z-API falhou para ${contact.name}:`, err);
        falhas++;
      }
    } catch (e) {
  ...
  if (enviados > 0 && falhas === 0) {
    ...
    toast(`✓ ${enviados} notificado${enviados > 1 ? "s" : ""}, ${falhas} falha${falhas > 1 ? "s" : ""}`);
  } else {
    toast("Não foi possível notificar pelo WhatsApp - verifique a conexão com Z-API");
  }
}
